...
export const MoviesPieChart = ({
    ...
    () =>
      characters
        .map((c) => ({
          ...
          y:
            (c.films?.length ?? 0) +
            (includeShortFilms ? c.shortFilms?.length ?? 0 : 0),
          films: c.films ?? [],
          shortFilms: c.shortFilms ?? [],
        }))
        .filter((c) => c.y > 0)
        .sort((a, b) => b.y - a.y),
    [characters, includeShortFilms]
  );
  ...
  const totalChartPages = Math.ceil(all.length / CHART_PAGE_SIZE)
  const data = all.slice(chartPage * CHART_PAGE_SIZE, (chartPage + 1) * CHART_PAGE_SIZE)
  ...
  const options: Highcharts.Options = {
    ...
    tooltip: {
      formatter() {
        ...
        const filmList = point.films.length
          ? `<br/><b>Films:</b> ${point.films.join(", ")}`
          : "";
        const shortFilmList =
          includeShortFilms && point.shortFilms.length
        ...
        const pct =
          point.percentage != null
            ...
            : `${point.y} films`;
        if (!filmList && !shortFilmList)
          return `<b>${point.name}</b><br/>${pct}`;
    ...
    accessibility: {
      ...
      point: {
        valueDescriptionFormat:
          "{point.name}: {point.percentage:.1f}% of total films",
      },
    },
    ...
    <div className="space-y-3">
            ...
            options={[
              {
                ...
                label: <PieChart className="w-3.5 h-3.5" />,
                ariaLabel: "Pie chart",
              },
              {
                value: "bar" as ChartType,
                label: <BarChart2 className="w-3.5 h-3.5" />,
                ariaLabel: "Bar chart",
              },
            ]}
          />
          <Button
            onClick={() => exportCharactersToXlsx(characters)}
            variant="secondary"
      ...
      {isLoading ? (
        ...
        <div className="flex items-center justify-center h-48 text-gray-400 dark:text-gray-500 text-sm">
          No film data available for the current characters.
        </div>
      ) : (
        <>
          <HighchartsReact immutable={false} highcharts={Highcharts} options={options} />
          {totalChartPages > 1 && (
